...
export function usersRouter(router: RouterType) {
  ...
  router.post("/users", async (request: any, env: Env & { USER_DO: DurableObjectNamespace }) => {
    ...
    const res = await stub.fetch(url.toString(), {
      ...
      body: JSON.stringify({ email: body.email, passwordHash: body.passwordHash }),
    });
    return res;
  });
  ...
  router.get("/users/me", async (request: any, env: Env) => {
    ...
    return new Response(JSON.stringify({ userId: entity.userId, email: entity.email, avatarUrl: entity.avatarUrl }), {
      status: 200,
      headers: { "Content-Type": "application/json" },
    });
  });
  ...
  router.put("/users/:userId", async (request: any, env: Env) => {
    ...
    const newPasswordHash = body?.passwordHash ?? existing.passwordHash;
    const newAvatarUrl = body?.avatarUrl ?? existing.avatarUrl;
    ...
    // If email changed, remove old email index and create new one
    const oldEmailLower = existing.email.toLowerCase();
    const newEmailLower = newEmail.toLowerCase();

    if (oldEmailLower !== newEmailLower) {
      // Overwrite old email index with a tombstone (optional) or leave it as-is for now.
      // For simplicity, we just write a new record for the new email.
      const emailEntity: TableEntity = {
        pk: `USER_EMAIL#${newEmailLower}`,
        sk: `PROFILE#${userId}`,
        entityType: "USER",
      ...
      await putEntity(env, emailEntity);
    }

    return new Response(JSON.stringify({ userId: updated.userId, email: updated.email, avatarUrl: updated.avatarUrl }), {
      status: 200,
      headers: { "Content-Type": "application/json" },
    });
  });
  ...
  router.delete("/users/:userId", async (request: any, env: Env & { WORKSPACE_DO: DurableObjectNamespace; WORKSPACE_CLEANUP_DO: DurableObjectNamespace }) => {
    const { userId } = request.params;
    const existing = await getEntity<UserEntity>(env, `USER#${userId}`, `PROFILE#${userId}`);
    if (!existing) {
      return new Response(JSON.stringify({ error: "User not found" }), { status: 404 });
